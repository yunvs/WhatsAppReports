...
def create_txt_reports() -> None:
	...
	for i, s in enumerate(db.sender):  # for each sender create a sender report
		ls = [y(0, 6, 18), "\n".join([
		...
		ls.append(f"In total {s} used {len(db.common_words[i])} distinct words and {len(db.common_emojis[i])} distinct emojis.")
...
def create_time_reports() -> None:
	...
	for i, s in enumerate(db.sender):  # for each sender create a sender report
		db.time_reports.append([
			...
			f"Most messages: {db.tstats.iat[i, 2]} ({db.tstats.iat[i, 3]} msg.)",
			f"Last message: {db.tstats.iat[i, 1]}",
			f"{s} sent at lest one message on {db.tstats.iat[i, 4]} days and no messages on {db.tstats.iat[i, 5]} days.",
			f"The longest period without any messages from {s} was {int(db.tstats.iat[i, 7])} days.",
			])
	...
	db.time_reports.append([
		...
		f"Most messages: {db.tstats.iat[i, 2]} ({db.tstats.iat[i, 3]} msg.)",
		f"Last message: {db.tstats.iat[i, 1]}",
		f"At lest one message sent on {db.tstats.iat[i, 4]} days.",
		f"Not a single message sent on {db.tstats.iat[i, 5]} days.",
		f"Longest period without any messages: {int(db.tstats.iat[i, 7])} days.",
		])
	return
