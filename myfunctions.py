...
def chats_to_list(string):
    ...
    for line in string:
        if check_for_pattern(line):
            ...
            msg_count += 1
            # if res.group(1) not in sender_dict:
            #     sender_dict[res.group(1)] = len(sender_dict)
            #     sender_texts.append([])
            # sender_texts[sender_dict[res.group(1)]].append(res.group(2))
        else:
            current_line = "\n" + line.strip("\n")
            chat_history[msg_count-1][2] += current_line
            # for i, sender in enumerate(senders): 
            #     if last_sender == sender:
            #         chat_senders[i].append(current_line)
    return chat_history
